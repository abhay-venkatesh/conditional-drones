...
parser.add_argument(
    "--mode", required=True, choices=["train", "test", "export"])
parser.add_argument(
...
# export options
parser.add_argument(
    "--output_filetype", default="png", choices=["png", "jpeg"])
a = parser.parse_args()
...
def deprocess_lab(L_chan, a_chan, b_chan):
    with tf.name_scope("deprocess_lab"):
        ...
        return tf.stack([(L_chan + 1) / 2 * 100, a_chan * 110, b_chan * 110],
                        axis=3)
...
def main():
    ...
    if a.mode == "export":
        ...
        input_image = tf.cond(
            tf.equal(tf.shape(input_image)[2],
                     1), lambda: tf.image.grayscale_to_rgb(input_image),
            lambda: input_image)
